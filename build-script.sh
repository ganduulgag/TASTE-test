...
# Generate code for OpenGEODE function crazyflie
cd "$SKELS"/crazyflie && opengeode --toAda crazyflie.pr system_structure.pr && cd $OLDPWD

cd "$SKELS" && rm -f crazyflie.zip && zip crazyflie crazyflie/* && cd $OLDPWD

cd "$SKELS" && rm -f led_control.zip && zip led_control led_control/* && cd $OLDPWD
...
cd "$CWD" && assert-builder-ocarina.py \
	...
	-o "$OUTPUTDIR" \
	--subAda crazyflie:"$SKELS"/crazyflie.zip \
	--subAda led_control:"$SKELS"/led_control.zip \
