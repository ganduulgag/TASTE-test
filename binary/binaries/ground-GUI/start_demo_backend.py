...
def setMsgQ():
    global tcId
    global msgQ
    tcId = PythonController.i_start_demo
    msgQ = True
...
def setUDP():
    global udp
    global tcId
    from _InterfaceEnum import i_ground_RI_start_demo
    tcId = i_ground_RI_start_demo
    udp = True
...
def setSharedLib(dll=None):
    ...
    global shared_lib
    global start_demo_via_shared_lib
    shared_lib = True
    start_demo_via_shared_lib = dll.ground_start_demo
...
def encode_ACN(asnVal):
    ...
    buffer = ASN1.DataStream(DV.T_Uint16_REQUIRED_BYTES_FOR_ACN_ENCODING)
...
def encode_uPER(asnVal):
    ...
    buffer = ASN1.DataStream(DV.T_Uint16_REQUIRED_BYTES_FOR_ENCODING)
...
def sendTC(tc):
    ...
    if msgQ:
        if checkConstraints(tc):
            try:
                PythonController.Invoke_start_demo(tc)
            except IOError:
                raise
    elif udp:
